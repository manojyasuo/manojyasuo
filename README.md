<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,50:2563eb,100:06b6d4&height=230&section=header&text=MANOJ%20K&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Full%20Stack%20Developer%20%7C%20Java%20%7C%20React%20%7C%20Spring%20Boot&descAlignY=60&descSize=17"/>

<br>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&pause=1200&color=2563EB&center=true&vCenter=true&width=700&lines=Full+Stack+Developer;Java+%7C+Spring+Boot+%7C+React;Building+Real-World+Web+Applications;Learning+%7C+Building+%7C+Improving" alt="Typing SVG" />

<br><br>

<a href="https://www.linkedin.com/in/manojyasuo/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:manojyasuo@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

<a href="https://github.com/manojyasuo">
<img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br><br>

<img src="https://komarev.com/ghpvc/?username=manojyasuo&style=flat-square&color=2563EB&label=PROFILE+VIEWS" />

</div>

---

# 👋 About Me

Hi, I'm **Manoj K**, a **B.Tech Information Technology graduate** and aspiring **Full Stack Developer**.

I enjoy building practical web applications using **Java, Spring Boot, React, JavaScript, Bootstrap, and MySQL**.

My main focus is understanding how the **frontend, backend, APIs, and database** work together to create complete web applications.

I am continuously improving my skills through projects, coding practice, and hands-on development.

> 💡 **My goal:** To become a strong Full Stack Developer and contribute to real-world software projects.

---

# 🧑‍💻 Quick Overview

<table>
<tr>
<td width="50%">

### 🎓 Education

**B.Tech – Information Technology**

K.S. Rangasamy College of Technology  
Tiruchengode

**2022 – 2026**

</td>

<td width="50%">

### 💼 Career

**Aspiring Full Stack Developer**

Interested in:

- Java Development
- Spring Boot
- React Development
- REST APIs
- Database Development

</td>
</tr>
</table>

---

# 🛠️ Technical Skills

## 💻 Programming Languages

<p>
<img src="https://skillicons.dev/icons?i=java,js,html,css" />
</p>

`Java` `JavaScript` `HTML5` `CSS3` `SQL`

---

## 🎨 Frontend Development

<p>
<img src="https://skillicons.dev/icons?i=react,bootstrap" />
</p>

- React.js
- React Hooks
- React Router
- Bootstrap
- Responsive Web Design
- Component-Based Development
- Axios
- JavaScript ES6+

---

## ⚙️ Backend Development

<p>
<img src="https://skillicons.dev/icons?i=spring" />
</p>

- Java
- Spring Boot
- Spring Security
- REST APIs
- JDBC
- JPA
- Hibernate
- Maven

---

## 🗄️ Database

<p>
<img src="https://skillicons.dev/icons?i=mysql" />
</p>

- MySQL
- SQL
- Database Design
- CRUD Operations
- Joins
- Subqueries
- Views
- Stored Procedures
- Triggers

---

## 🔧 Tools & Development

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

- Git
- GitHub
- VS Code
- Maven
- GitHub Projects

---

# 🚀 Featured Projects

## 🤖 AI-Based Task Reallocation & Employee Productivity System

### Full-Stack Employee Task Management Platform

A full-stack web application designed to intelligently **assign, monitor, and reallocate employee tasks** based on employee skills, workload, availability, and task priority.

### ✨ Main Features

| Feature | Description |
|---|---|
| 🔐 Authentication | Admin and Employee login |
| 👥 Employee Management | Admin creates and manages employee accounts |
| 🧠 Task Allocation | Assigns tasks based on suitability |
| ⚖️ Workload Balancing | Considers employee workload |
| 🔄 Task Reallocation | Reassigns pending tasks when required |
| 📅 Leave Management | Employee leave and availability management |
| 📊 Productivity | Tracks employee task performance |
| 🏆 Gamification | Points, badges and performance scoring |
| 📧 Notifications | Automated task and leave notifications |
| 📆 Calendar | Displays tasks and assignments by date |
| 👨‍💼 Dashboards | Separate Admin and Employee dashboards |

### 🧠 Core Algorithms

- Suitability Scoring Algorithm
- Greedy-Based Task Allocation
- Dynamic Task Reallocation
- Workload Balancing
- Reward-Based Performance Scoring
- Aggregation-Based Productivity Analysis

### 💻 Technology Stack

<p>

![Java](https://img.shields.io/badge/Java-17+-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.x-6DB33F?style=flat-square&logo=springboot&logoColor=white)
![React](https://img.shields.io/badge/React-18+-61DAFB?style=flat-square&logo=react&logoColor=black)
![MySQL](https://img.shields.io/badge/MySQL-Database-4479A1?style=flat-square&logo=mysql&logoColor=white)
![JPA](https://img.shields.io/badge/JPA%2FHibernate-ORM-59666C?style=flat-square)
![Spring Security](https://img.shields.io/badge/Spring%20Security-Security-6DB33F?style=flat-square&logo=springsecurity&logoColor=white)

</p>

### 🔗 Project

<a href="https://github.com/manojyasuo/AI-Based-Task-Reallocation-Employee-Productivity-System">

<img src="https://img.shields.io/badge/VIEW%20PROJECT-2563EB?style=for-the-badge&logo=github&logoColor=white" />

</a>

---

## 📸 Instagram Clone

### React-Based Social Media Application

A React-based Instagram-inspired application created to strengthen my understanding of **React components, state management, routing, reusable components, API handling, and responsive UI development**.

### ✨ Main Features

| Feature | Description |
|---|---|
| 🏠 Home | Instagram-style home interface |
| 📖 Stories | Stories interface |
| 🎬 Reels | Reels interface |
| 👤 Profile | User profile management |
| ✏️ Profile Update | Update user profile information |
| 📝 Username | Update username |
| 👥 Followers | View listed followers |
| 🌓 Theme | Light and Dark mode |
| 📱 Responsive | Responsive user interface |
| 🔀 Routing | Client-side navigation |

### 💻 Technology Stack

<p>

![React](https://img.shields.io/badge/React.js-61DAFB?style=flat-square&logo=react&logoColor=black)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat-square&logo=bootstrap&logoColor=white)
![Axios](https://img.shields.io/badge/Axios-5A29E4?style=flat-square&logo=axios&logoColor=white)

</p>

### 🔗 Project

> ⚠️ Replace the repository URL below with your **actual Instagram Clone repository URL** if the repository name is different.

<a href="https://github.com/manojyasuo/instagram-clone-react.git">


<img src="https://img.shields.io/badge/VIEW%20PROJECT-2563EB?style=for-the-badge&logo=github&logoColor=white" />

</a>

---

# 💼 Internship Experience

## Full Stack Java Intern

### Tech Volt Solutions Pvt. Ltd. — Coimbatore

During my internship, I gained practical exposure to **Java full-stack development** and learned how frontend, backend, and database components work together in a web application.

### Technologies Used

`Java` `HTML` `CSS` `JavaScript` `SQL` `JDBC`

### Key Learning

- Java application development
- Database connectivity using JDBC
- SQL and database operations
- Frontend development
- Backend development concepts
- Understanding complete application flow

---

# 🎓 Education

### K.S. Rangasamy College of Technology

**B.Tech – Information Technology**

📍 Tiruchengode, Tamil Nadu

📅 **2022 – 2026**

🎓 **Graduated: May 2026**

---

# 📚 Currently Learning

<div align="center">

| JavaScript | React | Spring Boot |
|:---:|:---:|:---:|
| Advanced JavaScript | React Hooks | REST APIs |
| ES6+ | Context API | Spring Security |
| Async/Await | React Router | JPA/Hibernate |
| Promises | State Management | Backend Architecture |

</div>

### 💻 Problem Solving

Currently improving my problem-solving skills through:

- Data Structures & Algorithms
- Java coding practice
- SQL problems
- Logical and aptitude problems

---

# 🎯 Career Objective

I am looking for an opportunity as a **Full Stack Developer** where I can apply my knowledge of:

`Java` `Spring Boot` `React` `JavaScript` `MySQL`

and contribute to real-world applications while learning from experienced developers and continuously improving my technical skills.

---


# 📊 GitHub

<div align="center">

<a href="https://github.com/manojyasuo">
  <img src="https://img.shields.io/badge/GitHub-manojyasuo-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://github.com/manojyasuo?tab=repositories">
  <img src="https://img.shields.io/badge/Repositories-View%20Projects-2563EB?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br><br>

<a href="https://github.com/manojyasuo">
  <img src="https://img.shields.io/github/followers/manojyasuo?style=flat-square&label=Followers&logo=github" />
</a>

<a href="https://github.com/manojyasuo">
  <img src="https://img.shields.io/github/stars/manojyasuo?style=flat-square&label=Total%20Stars&logo=github" />
</a>

</div>

<br>

> 💻 Check out my repositories to explore my projects and development work.
---

# 🤝 Let's Connect

<div align="center">

<a href="https://www.linkedin.com/in/manojyasuo/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:manojyasuo@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

<a href="https://github.com/manojyasuo">
<img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

</div>

<br>

<div align="center">

### Thanks for visiting my profile! 👋

**Building • Learning • Improving**

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f172a,50:2563eb,100:06b6d4&height=120&section=footer"/>

</div>
